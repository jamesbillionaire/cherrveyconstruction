...
export function ProjectVisual({ project, className }: ProjectVisualProps) {
  if (project.image) {
    return <div className={cn("relative aspect-[16/10] overflow-hidden bg-brand-ink", className)}><Image src={project.image} alt={`${project.title} for ${project.client}`} fill className="object-cover" sizes="(max-width: 768px) 100vw, 50vw" /></div>;
  }
  ...
  return (
    <div className={cn("cv-project-graphic aspect-[16/10]", className)} aria-hidden="true">
